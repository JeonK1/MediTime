...
class NoticeFragment : Fragment() {
    ...
    companion object{
        // 상수 선언
        private const val ADD_MEDICINE = 103
        private const val ADD_MEDICINE_MODIFY = 104
    }
    ...
    private fun recyclerViewInit() {
        mediAdapter = NoticeAdapter(dbCreater.get_noticeinfo_all())
    ...
    private fun clickListenerInit() {
        ...
        mediAdapter.itemClickListener = object : NoticeAdapter.OnItemClickListener {
            override fun onItemClick(
                ...
                // 아이템 자체를 클릭했을 때
                val set_cycle = mediAdapter.items[position].set_cycle
                val re_type = mediAdapter.items[position].re_type
                val re_cycle = mediAdapter.items[position].re_cycle
                val alarm_cnt = mediAdapter.items[position].time_list.size
                ...
                // recyclerView
                val alarmSetDlgAdapter = AlarmSetDlgAdapter(mediAdapter.items[position].time_list)
                ...
                if (set_cycle == 0)
                    ...
                    when (re_type) {
                        ...
                        1 -> mDialogView.tv_alarmsetdlg_cycle_type.text =
                            "${re_cycle}일 간격" // 일 반복
                        2 -> mDialogView.tv_alarmsetdlg_cycle_type.text =
                ...
                mDialogView.btn_alarmsetdlg_save.setOnClickListener {
                    ...
                    // DB에 적용
                    val medi_no = mediAdapter.items[position].medi_no
                ...
                mDialogView.tv_alarmsetdlg_delete.setOnClickListener {
                    // 삭제 버튼 클릭 시
                    val medi_no = mediAdapter.items[position].medi_no
                    val medi_name = mediAdapter.items[position].medi_name
                ...
                mDialogView.tv_alarmsetdlg_modify.setOnClickListener {
                    ...
                    val bundle = Bundle()
                    bundle.putSerializable("noticeInfo2", mediAdapter.items[position])
