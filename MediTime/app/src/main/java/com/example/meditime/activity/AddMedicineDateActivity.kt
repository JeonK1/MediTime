...
class AddMedicineDateActivity : AppCompatActivity() {
    ...
    companion object{
        // 상수 선언
        private const val PICKER_MAX_VALUE = 10
        private const val PICKER_MIN_VALUE = 1
        private const val ADD_MEDICINE_TIME = 200
    }
    ...
    private fun okButtonInit() {
        ...
        if(et_addmedidate_name.text.isEmpty()) {
    ...
    private fun listenerInit() {
        ...
        rl_addmedidate_cycletype.setOnClickListener {
            ...
            cycleDialogView.btn_cyclepickdig_type1.setOnClickListener {
                ...
                cur_dayofweek_flag = if(cur_noticeInfo.re_type==0){
                    // 요일
                    DowConverterFactory.convert_int_to_arrayList(cur_noticeInfo.re_cycle)
                } else {
                    // 요일이 아님 (요일 초기화)
                    DowConverterFactory.convert_int_to_arrayList(0)
                }
    ...
    private fun setButtonType(is_everyday: Boolean){
        if(is_everyday){
            // 매일 버튼 눌렀을 때
    ...
    private fun setDayTypeTextView(idx: Int, mDialogView: View){
    ...
    private fun updateTypeTextView(type:Int){
        // type 쪽 글씨 변경 함수
        when (type) {
            0 -> tv_addmedidate_cycletype.text = "특정 요일"
